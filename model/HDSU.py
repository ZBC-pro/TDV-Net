...
class HDSU(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        assert in_channels % 2 == 0, f"Input channels {in_channels} cannot be split into 2 groups."

        self.split_channels = in_channels // 2

        self.g1_conv = nn.Conv2d(self.split_channels, self.split_channels, kernel_size=1, bias=False)
        self.g1_bn = nn.BatchNorm2d(self.split_channels)
        self.g1_act = nn.ReLU(inplace=True)

        self.g2_offset = nn.Conv2d(self.split_channels, 18, kernel_size=3, padding=1)
        self.g2_deform = DeformConv2d(self.split_channels, self.split_channels, kernel_size=3, padding=1)
        self.g2_bn = nn.BatchNorm2d(self.split_channels)
        self.g2_act = nn.ReLU(inplace=True)
    ...
    def forward(self, x):
        ...
        y1 = self.g1_act(self.g1_bn(self.g1_conv(x1)))

        g2_input = x2 + y1

        offsets = self.g2_offset(g2_input)

        y2 = self.g2_deform(g2_input, offsets)
        y2 = self.g2_act(self.g2_bn(y2))

        y_cat = torch.cat([y1, y2], dim=1)
        out = self.fusion_act(self.fusion_bn(self.fusion_conv(y_cat)))

        return out + self.residual_match(x)

class U_HDSU(nn.Module):

    def __init__(self, options: dict):
        super().__init__()

        print("\nInitializing U_Deform (with Paper-Aligned HDSU Module)...")
