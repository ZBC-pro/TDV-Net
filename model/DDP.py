...
class HDSU(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, num_groups: int = 2):
        super().__init__()
        ...
        for g in range(num_groups):
            if g == 0:
                ...
                self.offset_convs.append(
                    nn.Conv2d(self.group_channels, 2 * kernel_size * kernel_size, 
                             kernel_size=3, padding=1)
                )
                self.group_convs.append(
                    DeformConv2d(self.group_channels, self.group_channels, 
                                kernel_size=kernel_size, padding=padding)
                )
            
            self.group_bns.append(nn.BatchNorm2d(self.group_channels))
        ...
        if in_channels != out_channels:
            self.residual_match = nn.Sequential(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for g, (x_g, conv, bn) in enumerate(zip(x_groups, self.group_convs, self.group_bns)):
            if g == 0:
                                                
                y_g = self.act(bn(conv(x_g)))
            else:
                                                              
                g_input = x_g + y_prev
                
                                                          
                offset = self.offset_convs[g](g_input)
                y_g = self.act(bn(conv(g_input, offset)))
            
            y_groups.append(y_g)
            y_prev = y_g
        
                              
        y_cat = torch.cat(y_groups, dim=1)
        out = self.act(self.fusion_bn(self.fusion_conv(y_cat)))
        
                             
        return out + self.residual_match(x)
...
class FractionalGaborFilter(nn.Module):
    ...
    def _compute_gabor_envelope(self, orientation: torch.Tensor, scale: torch.Tensor) -> torch.Tensor:
        ...
        envelope = torch.exp(-(x_theta ** 2 + y_theta ** 2) / (2 * sigma ** 2 + 1e-8))
        ...
        for c in range(self.num_prototypes):
            ...
            for v in range(self.num_scales):
                ...
                for u in range(self.num_orientations):
                    ...
                    norm_factor = magnitude.sum() + 1e-8
...
class TextureAdaptiveConv(nn.Module):
    ...
    def __init__(
        ...
        self.base_kernel = nn.Parameter(
            torch.randn(out_channels_per_group, in_channels, kernel_size, kernel_size) * 0.02
        )
        
        self.frgt_filters = FractionalGaborFilter(
    ...
    def forward(self, x: torch.Tensor, scale_idx: int) -> torch.Tensor:
        ...
        for c in range(self.num_prototypes):
            for u in range(self.num_orientations):
                ...
                modulated = torch.sqrt(modulated_real ** 2 + modulated_imag ** 2 + 1e-8)
...
class MFSU(nn.Module):
    ...
    def __init__(
        ...
        assert in_channels % num_scales == 0, \
            f"in_channels ({in_channels}) must be divisible by num_scales ({num_scales})"
        ...
        self.texture_convs = nn.ModuleList([
            TextureAdaptiveConv(
                in_channels=self.channels_per_scale,
                out_channels_per_group=self.out_per_group,
                kernel_size=kernel_size,
                num_prototypes=num_prototypes,
                num_orientations=num_orientations,
                num_scales=num_scales,
                alpha_init=alpha_tensor
            )
        ...
        self.proj = nn.Sequential(
            nn.Conv2d(texture_out_channels, out_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(out_channels),
...
class DDPBlock(nn.Module):
    ...
    def __init__(
        ...
        freq_channels = int(in_channels * alpha)
        spatial_channels = in_channels - freq_channels
        
                                                         
        spatial_channels = (spatial_channels // num_groups) * num_groups
        if spatial_channels == 0:
            spatial_channels = num_groups
        
                                                      
        freq_channels = (freq_channels // num_scales) * num_scales
        if freq_channels == 0:
            freq_channels = num_scales
        ...
        total_split_channels = spatial_channels + freq_channels
        self.input_proj = nn.Sequential(
            nn.Conv2d(in_channels, total_split_channels, kernel_size=1, bias=False),
            nn.BatchNorm2d(total_split_channels),
            nn.ReLU(inplace=True)
        ) if in_channels != total_split_channels else nn.Identity()
        ...
        fused_channels = spatial_channels + freq_channels
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x_spatial = x[:, :self.spatial_channels]
        x_freq = x[:, self.spatial_channels:]
        
                             
        y_spatial = self.dspu(x_spatial)            
        y_freq = self.fpu(x_freq)                    
        
                                              
        y_agg = torch.cat([y_spatial, y_freq], dim=1)
        out = self.fusion(y_agg)
...
class U_DDP(nn.Module):
    ...
    def __init__(self, options: dict):
        ...
        print(f"SAR channels: {n_sar}, Aux channels: {n_aux}")
        
                               
        f = [32, 32, 64, 128, 256]
        print(f"Channel config: {f}")
        
                        
        self.alpha = options.get('dsf_alpha', 0.5)               
        self.num_groups = options.get('num_groups', 2)               
    ...
    def forward(self, x):
        ...
        return {
            'SIC': self.sic_head(d1),
            'SOD': self.sod_head(d1),
            'FLOE': self.floe_head(d1)
        }
