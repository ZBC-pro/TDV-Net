...
try:
    from vss_ssm import VSS
    HAS_VSS = True
    print(">> VSS-SSM library detected. Using hardware-accelerated VSS.")
except ImportError:
    HAS_VSS = False
    print(">> Warning: 'vss_ssm' not found. Using PyTorch Self-Attention placeholder.")
...
class VSSLayer(nn.Module):
    ...
    def __init__(self, channels, d_state=16, d_conv=4, expand=2):
        ...
        if HAS_VSS:
            ...
            self.vss = nn.MultiheadAttention(embed_dim=channels, num_heads=4, batch_first=True)
    ...
    def forward(self, x):
        ...
        if HAS_VSS:
            ...
            out, _ = self.vss(x_norm, x_norm, x_norm)
...
class U_vss(nn.Module):

    def __init__(self, options: dict):
        super().__init__()

        print("\nInitializing HybridDualStreamUNet (Standard SAR + Delayed Aux VSS)...")
        ...
        self.sar_enc2 = DoubleConvolution(f[0], f[1])

        self.aux_enc2 = DoubleConvolution(f[0], f[1])
        self.down2 = DownSample()


        self.sar_enc3 = DoubleConvolution(f[1], f[2])
        ...
        self.sar_enc4 = DoubleConvolution(f[2], f[3])

        self.aux_enc4 = nn.Sequential(
            nn.Conv2d(f[2], f[3], 1),
            VSSLayer(f[3])
        )
